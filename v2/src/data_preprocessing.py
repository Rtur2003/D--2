...
from config import LABELS_CSV, IMG_SIZE, CLASS_NAMES


def load_labels() -> pd.DataFrame:
    """v2 labels.csv: path (mutlak), label, source, patient_id"""
    df = pd.read_csv(LABELS_CSV)
    df = df.rename(columns={"path": "image_path"})

    missing = df[~df["image_path"].apply(os.path.exists)]
    if len(missing) > 0:
        print(f"[WARNING] {len(missing)} gorsel bulunamadi")

    print(f"[DATA] Toplam: {len(df)} goruntu")
